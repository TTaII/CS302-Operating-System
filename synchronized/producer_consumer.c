...
void *producer(void *nothing) {
	int item;
	unsigned int s;		// needed by thread-safe rand_r() 局部变量初始化但不赋值，获得的是机器垃圾，即该地址上上一个程序使用过后的值

	while(1) {
		pthread_mutex_lock(&mutex);	// start of mutual exclusion.

			if(IS_FULL(head, tail, N)) // 如果满了，等待full锁的释放
				pthread_cond_wait(&full, &mutex);	// wait and unlock mutual exclusion.

			if(IS_EMPTY(head, tail, N)) // 如果是空的，释放锁并唤醒consumer(ready)，但是mutex没释放，所以下面生产一个item还是原子性
				pthread_cond_signal(&empty);		// wake up the consumer.
			// 既不满也不空就不用管理empty和full锁，在mutex锁下正常生产就行
...
void *consumer(void *nothing) {
	...
	while(1) {
		pthread_mutex_lock(&mutex);	// start of mutual exclusion.
			...
			if(IS_EMPTY(head, tail, N)) // 如果空了，等待empty锁的释放
			...
			// 既不满也不空就不用管理empty和full锁，在mutex锁下正常消费就行
